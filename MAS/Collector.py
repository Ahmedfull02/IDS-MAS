...
class CollectorAgent(Agent):
    class ReceiveBehaviour(CyclicBehaviour):
        async def on_start(self):
            self.received = []
            # Structure: { "data_id": { ...data..., "_first_seen": datetime } }
            self.pending_data = {}
            ...
            if msg:
                try:
                    ...
                    if data_id not in self.pending_data:
                        ...
                        self.pending_data[data_id]["_first_seen"] = datetime.datetime.now()
                    else:
                        self.pending_data[data_id].update(data)
                ...
                # No message received, loop will restart and trigger check_timeouts()
                pass
        async def process_and_send(self, data_id, merged_data):
            # Remove the timestamp marker so it isn't sent to dashboard
            if "_first_seen" in merged_data:
                del merged_data["_first_seen"]
            ...
            # so that we don't accidentally flag partial data as an attack unless an attack was actually seen.
        ...
        async def check_timeouts(self):
            ...
            for data_id, data in self.pending_data.items():
                first_seen = data.get("_first_seen")
