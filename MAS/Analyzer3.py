...
MODEL_PATH = "MAS/model/GB/model.pkl"
ENCODER_PATH = "MAS/model/GB/encoder_y.pkl" 
METRICS_PATH = "MAS/model/GB/metric_results.json"
IMPORTANCE8PATH = "MAS/model/GB/importance_df.json"

AGENT6 = "agent6@localhost"

class GBAnalyzerAgent(Agent):
        ...
        async def run(self):
            ...
            if msg:
                if msg.body is not None:
                    ...
                    print("[XGBAnalyzer] Received message with no body.")
                    return
                ...
                print(f"[GBAnalyzer] Received data for ID {data_id}")
                ...
                print(f"[GBAnalyzer] Prediction: {prediction} for ID {data_id}")

                # Send data to dashboard agent 
                new_msg = Message(to=AGENT6)
                data_processed.update({"GBPred": prediction})
                new_msg.body = json.dumps(data_processed)
                await self.send(new_msg)
                
                print(f"[GBAnalyzer] Prediction sent to Collector (ID: {data_id}, Prediction: {prediction})")
    ...
    async def setup(self):
        ...
        print(f"\n\n\n[GBAnalyzer] Agent {self.jid} started\n\n\n")